# AGENTS.md — 「爱教学」后端编码智能体指导手册（Go + Gin）
...
1. **编码助手（GLM 5.3 + OpenCode）**：只承接执行类任务；每次产出附带实现说明、修改点清单与测试脚本；**代码合入前必须经成员四（后端·胡凯翔）初审，再经成员一/成员三复审**，处置结论记入 AI 互动记录表；
2. **架构助手（DeepSeek V4 Pro）**：仅产出方案选项与利弊对比；最终技术方案由成员二（前端）与成员四（后端）共同确定并记录理由；
3. **范围守卫**：涉及 §2.2 Won't 清单的请求先亮红灯说明，不得直接实现；
4. **接口变更纪律**：任何 DTO 字段增删改，必须先改本文件 §8 并通知前端同步 `types/`，再做代码实现——**契约先行**；
5. **Sprint 2 预告**：ASR 智能体接入（音频上传→转写）届时新增 `recordings`/`transcripts` 表与独立路由组，本轮不预建代码，仅在 model 层保持可扩展。

---

*文档版本：v1.0（Sprint 1 设计阶段产出）· 维护人：成员四（胡凯翔，后端架构）· 接口契约评审：成员二（刘子杰，前端）· DRI 复核：成员一（徐仕杰）*
