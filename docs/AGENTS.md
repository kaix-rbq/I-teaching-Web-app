...
**当前状态**：Sprint 1（第 1 轮迭代），主题"查课程"。团队为"4+n"模式（4 名学生 + 4 个 AI 智能体），采用 Scrum 方法，项目周期 6 周。
...
结合"4+n"团队模式（编码助手 GLM 5.3 + OpenCode / 架构助手 DeepSeek V4 Pro）：

1. **编码助手**：只承接执行类编码任务；每项产出附带实现说明 + 修改点清单 + 必要测试脚本；**代码合入前必须经成员二（前端）初审 + 成员一/成员三复审**；
2. **架构助手**：仅产出方案选项与利弊对比，最终技术决策由成员二/成员四共同确定；
3. **范围守卫**：任何请求若涉及 §2.2 的 Won't 清单，先亮红灯说明，不得直接实现；
4. **对话记录**：每次人-智能体互动的结论（采纳/修改/否决 + 理由）需随做随记，汇总至团队 AI 互动记录表（表 3）。

---

*文档版本：v1.0（Sprint 1 设计阶段产出）· 维护人：成员二（刘子杰，前端架构）· 评审：全员*
