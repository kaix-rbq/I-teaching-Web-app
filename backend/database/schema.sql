-- 「爱教学」Sprint 1 建库建表脚本
-- 事实源：docs/MySQL数据库创建指导.md §2 / §4
-- 执行：mysql -u root -p < database/schema.sql
-- 说明：GORM 不使用 AutoMigrate，表结构变更 = 改本文件 + 改 internal/model + PR 说明。
...
CREATE USER IF NOT EXISTS 'aijiaoxue'@'localhost' IDENTIFIED BY 'aijiaoxue_dev';
GRANT ALL PRIVILEGES ON `aijiaoxue`.* TO 'aijiaoxue'@'%';
GRANT ALL PRIVILEGES ON `aijiaoxue`.* TO 'aijiaoxue'@'localhost';
FLUSH PRIVILEGES;
